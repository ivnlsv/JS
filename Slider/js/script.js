...
function initBtn() {
  aparts.forEach((image, index) => {
    const button = `<div class="btn n${index} ${
      index === 0 ? "active" : ""
    }" data-index="${index}">${
      aparts[index].city + " " + aparts[index].apartName
    }</div>`;
...
function initCities() {
  const cityDiv = `<div class="city__title">${aparts[0].city}</div>`;
  const apartDiv = `<div class="apart__title">${aparts[0].apartName}</div>`;
  const areaDiv = `<div class="area__title">${aparts[0].area}</div>`;
  const repTimeDiv = `<div class="reptime__title">${aparts[0].repTime}</div>`;
  const repCostDiv = `<div class="repcost__title">${aparts[0].repCost}</div>`;
...
function changeCity(num) {
  const cityTitle = cityLocation.querySelector(".city__title");
  cityTitle.innerText = aparts[num].city;
  const apartTitle = cityLocation.querySelector(".apart__title");
  apartTitle.innerText = aparts[num].apartName;
  const areaTitle = apartArea.querySelector(".area__title");
  areaTitle.innerText = aparts[num].area;
  const repTimeTitle = repairTime.querySelector(".reptime__title");
  repTimeTitle.innerText = aparts[num].repTime;
  const repCostTitle = repairCost.querySelector(".repcost__title");
